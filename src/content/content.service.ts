...
export class ContentService {
  ...
  // ====== BecomeHost ======
  async createBecomeHost(data: CreateAboutDto, file?: Express.Multer.File) {
    // When receiving multipart/form-data, nested objects like `seo` arrive as strings
    const normalized: any =
      data && typeof (data as any).seo === 'string'
        ? { ...data, seo: safeJsonParse((data as any).seo) }
        : data;
    const parsed = createAboutSchema.safeParse(normalized);
    if (!parsed.success) throw new NotAcceptableException(parsed.error.errors);

    // If any BecomeHost exists, update it instead of creating a new one
    const existingAny = await this.prisma.becomehost.findFirst();
    if (existingAny) {
      this.logger.log(
        `About exists (${existingAny.id}), updating instead of creating`,
      );
      return this.updateBecomeHost(
        existingAny.id,
        parsed.data as UpdateAboutDto,
        file,
      );
    }
    ...
    let imageKey: string | undefined;
    if (file) {
      const upload = await this.uploadService.uploadFile(file, 'about');
      imageKey = upload.Key;
    }

    const seo = await this.prisma.seoMetadata.create({
      data: { ...parsed.data.seo, ogImage: imageKey, ogType: 'website' },
    });

    const created = await this.prisma.becomehost.create({
      data: {
      ...
      include: { seo: true, faq: true },
    });
    this.logger.log(`Created AboutPage ${created.id}`);
    return {
      status: true,
      data: created,
  ...
  async updateBecomeHost(
    ...
    const about = await this.prisma.becomehost.findUnique({ where: { id } });
    if (!about) throw new NotFoundException('About page not found');

    let imageKey = about.image;
    if (file) {
    ...
    if (parsed.data.seo) {
      await this.prisma.seoMetadata.update({
        where: { id: about.seoId },
        data: { ...parsed.data.seo, ogImage: imageKey, ogType: 'website' },
      });
    }

    const updated = await this.prisma.becomehost.update({
      where: { id },
      data: {
        heading: parsed.data.heading,
        subHeading: parsed.data.subHeading,
    ...
    this.logger.log(`Updated BecomeHost ${id}`);
    return {
      status: true,
      data: updated,
      message: 'BecomeHost updated successfully.',
    };
  }
  ...
  async getBecomeHosts() {
    const data = await this.prisma.becomehost.findFirst({
      include: { seo: true, faq: true, cards: true },
      orderBy: { updatedAt: 'desc' },
    });
    return { status: true, data };
  }

  async getBecomeHostPublic() {
    const [data, setting, faq] = await this.prisma.$transaction([
      this.prisma.becomehost.findFirst({
        select: {
          id: true,
          heading: true,
          subHeading: true,
  ...
  // ===== Card (no SEO) =====
  async createCard(data: CreateCardDto, file?: Express.Multer.File) {
    const parsed = createCardSchema.safeParse(data);
    if (!parsed.success) throw new NotAcceptableException(parsed.error.errors);
    let imageKey = parsed.data.image;
    if (file) {
      const upload = await this.uploadService.uploadFile(file, 'card');
      imageKey = upload.Key;
    }
    ...
    if (parsed.data.becomehostId) {
      const host = await this.prisma.becomehost.findUnique({
        where: { id: parsed.data.becomehostId },
        select: { id: true },
      });
      if (!host) {
        throw new NotAcceptableException(
          'Invalid becomehostId: record not found',
        );
      }
      createData.becomehost = { connect: { id: parsed.data.becomehostId } };
    }
  ...
  async updateCard(
    ...
    if (parsed.data.becomehostId) {
      ...
      updateData.becomehost = { connect: { id: parsed.data.becomehostId } };
    }
  ...
  async getCardsAdmin(becomehostId?: string) {
    const data = await this.prisma.card.findMany({
      where: {
        becomehostId: becomehostId,
      },
      orderBy: { updatedAt: 'desc' },
    });
    return { status: true, data };
  ...
  async getCards(becomehostId?: string) {
    const data = await this.prisma.card.findMany({
      where: {
        isActive: true,
        becomehostId: becomehostId,
