...
export class EventProcessor extends WorkerHost implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    this.logger.log(
      'EventProcessor ready to process jobs from queue:',
      QUEUES.EVENT,
    );
      ...
      try {
        ...
        if (this.worker) {
          try {
            ...
            this.logger.log(
              'Worker Redis connection test - attempting to check connection...',
            );
            ...
            this.logger.error(
              'Redis connection error in EventProcessor:',
              redisError,
            );
          }
        }
  ...
  async process(job: Job) {
    ...
    try {
      ...
      if (job.name === 'test-job') {
        this.logger.log(
          `Test job processed successfully: ${job.data?.message}`,
        );
      ...
      if (job.data?.experienceId) {
        // const result = await this.eventService.initiateEvent(
        //   job.data.experienceId,
        // );
        const duration = Date.now() - startTime;
        this.logger.log(
          `Legacy event processed successfully in ${duration}ms: ${jobContext}`,
        );
  ...
  private async processRecurringEvent(job: Job, jobContext: string) {
    ...
    this.logger.debug(
      `Processing recurring event: exp=${experienceId}, targetDate=${targetDateIso}, batch=${batchId}`,
    );
    ...
    //   throw new Error(
    //     'generateEventsForTargetDates method not found on EventService',
    //   );
    // }
    ...
    try {
      if (targetDateIso) {
        ...
        this.logger.debug(
          `Processing single target date: ${targetDateIso} for exp=${experienceId}`,
        );
      } else {
        ...
        this.logger.debug(
          `Processing legacy targets (+16, +31 days) for exp=${experienceId}`,
        );
      }
    } catch (dateError) {
      throw new Error(
        `Invalid target date '${targetDateIso}': ${(dateError as Error)?.message}`,
      );
    }

    this.logger.debug(
      `About to call generateEventsForTargetDates with exp=${experienceId}, targets=${targets.length}`,
    );
    ...
    try {
      ...
      this.logger.error(
        `Error calling generateEventsForTargetDates: ${(serviceError as Error)?.message}`,
  ...
  private handleJobError(
    ...
    if (
      ...
      this.logger.warn(
        `Idempotent conflict (non-fatal): ${jobContext} - ${msg}`,
      );
    ...
    if (
      ...
      this.logger.warn(
        `Validation error (non-retryable): ${jobContext} - ${msg}`,
      );
    ...
    if (isRetryable) {
      ...
      this.logger.error(
        `Permanent failure: ${jobContext} - ${msg}`,
  ...
  onCompleted(job: Job, result: any) {
    ...
    this.logger.log(
      `Job ${job.id} completed: created=${created}, duration=${duration}ms`,
    );
  }
